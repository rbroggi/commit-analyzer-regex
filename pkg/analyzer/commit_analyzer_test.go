...
func TestAnnotations(t *testing.T) {
	...
	defaultAnalyzer.Init(map[string]string{})
...
func TestRegexAnalyzer(t *testing.T) {
	testCases := []struct {
		...
		{
			createRawCommit("j", `feat: Merge remote-tracking branch 'origin/master' into migration_jenkins_to_gitlab_ci_goModDefaultPipeline_phase_3`),
			"feat",
			"",
			&semrel.Change{Major: false, Minor: true, Patch: false},
		},
	...
	defaultAnalyzer := &DefaultCommitAnalyzer{}
	defaultAnalyzer.Init(map[string]string{"minor": "feat", "patch": "build|ci|fix|perf|refactor|test|chore"})
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("AnalyzeCommitMessage: %s", tc.RawCommit.RawMessage), func(t *testing.T) {
			require.True(t, compareCommit(defaultAnalyzer.analyzeSingleCommit(tc.RawCommit), tc.Type, tc.Scope, tc.Change))
		})
